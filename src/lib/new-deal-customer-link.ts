...
export function getCustomerIngestionTabUrl(
  customerId: string,
  queueItemId: string,
  opts?: { from?: string },
): string {
  const params = new URLSearchParams({ tab: "ingestion", queueItemId });
  if (opts?.from) params.set("from", opts.from);
  return `/customers/${customerId}?${params.toString()}`;
}

/** Approver / post-submit deep link — lands on Invoice Preview inside Ingestion. */
export function getCustomerIngestionInvoicePreviewUrl(
  customerId: string,
  queueItemId: string,
  opts?: { from?: string },
): string {
...
/** Land on the customer Ingestion tab after linking/creating a customer. */
export function openQueueIngestionTab(
  customerId: string,
  queueItemId: string,
  navigate: NavigateFunction,
) {
  navigate(getCustomerIngestionTabUrl(customerId, queueItemId, { from: "workbench" }));
}

/** @deprecated Use openQueueIngestionTab — kept for any contract-tab deep links */
