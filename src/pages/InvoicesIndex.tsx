...
function toRow(inv: typeof invoices[0], customerLookup: typeof customers): InvoiceGroupRow {
  const c = customerLookup.find((cu) => cu.id === inv.customerId);
  return {
    invoiceId: inv.id,
...
function buildGroups(source: typeof invoices, customerLookup: typeof customers) {
  ...
  for (const inv of source) {
    const row = toRow(inv, customerLookup);

    if (inv.status === "Pending Review" || inv.status === "Pending Approval") groups["pending-review"].push(row);
...
export function InvoicesIndex() {
  ...
  const groups = useMemo(() => buildGroups(invoicesView, customersMerged), [invoicesView, customersMerged]);
        ...
        <div className="flex flex-col gap-5 px-6 pt-5 pb-7">
          ...
          <ListTable columns={listColumns} resultCount={filtered.length}>
            {filtered.map((inv) => {
              ...
              return (
                <ListRow key={inv.id} onClick={() => goToShell(toRow(inv, customersMerged))}>
